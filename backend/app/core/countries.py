"""
Static ISO 3166-1 country list used as the source of truth for the
onboarding country dropdown.

Why static rather than a live third-party call: the country dropdown is
a hard blocker in the onboarding wizard (you cannot advance past the
location step without picking one), and the previous implementation
proxied countries.dev — which returned an HTML 404 for the endpoint it
called, so the dropdown was permanently empty and the wizard could never
be completed. A vendored list can't go down, can't rate-limit, needs no
network round trip, and countries change roughly never.

`flag` is the emoji (derived from the alpha-2 code, no asset needed) and
`flag_url` points at flagcdn for clients that prefer an image.
"""

# (alpha-2, name) — ISO 3166-1, English short names.
_ISO_3166: list[tuple[str, str]] = [
...
def _flag_emoji(code: str) -> str:
    return "".join(chr(ord(ch) + _REGIONAL_INDICATOR_OFFSET) for ch in code.upper())


COUNTRIES: list[dict] = sorted(
    (
        {
            "name": name,
            "code": code,
...
def search_countries(q: str, limit: int = 10) -> list[dict]:
    needle = q.strip().lower()
    if not needle:
        return []
    starts = [c for c in COUNTRIES if c["name"].lower().startswith(needle)]
    contains = [c for c in COUNTRIES if needle in c["name"].lower() and c not in starts]
